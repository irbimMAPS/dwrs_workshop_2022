...
# General settings ----
#-----------------------------------------
setwd("C:/Users/e.armelloni/OneDrive/Lavoro/AIS/Lavori/Lavori_2022/GFCM_workshop/dwrs_workshop_2022")
...
# Load data ----
#-----------------------------------------
setwd("C:/Users/e.armelloni/OneDrive/Lavoro/AIS/Lavori/Lavori_2022/GFCM_workshop/dwrs_workshop_2022/practical_session_1")
...
all_dat<-read.csv("practical_session_1/AIS_data_processing/data/datatest.csv") # load all data
...
<p> ![Figure 1: Conceptual framework of the worflow used in AIS data processing](practical_session_1/AIS_data_processing/documents/conceptual_workflow.jpeg)
...
```{r}
data = read.csv("practical_session_1/AIS_data_processing/data/datatest.csv")
head(data)
```
Where:
...
<p> In particular, a k-means algorithms is performed on trip-specific speed values using 5 different sets of centroids that were a priori defined by experts for each fishing gear type (two for OTB, PTM, TBB and PS), the centroids will represent the different speed profiles of fishing behavioural states. The dbscan algorithm is applied on the latitude and longitude data, with two different sets of neighborhood radii (??) and a number of minimum points in the ?? region. The proportions of points falling within each of the identified clusters are used to label the fishing trip as positive or negative to each of the investigated gears.
...
all_dat<-read.csv("practical_session_1/AIS_data_processing/data/datatest.csv")
