...
outdir="AIS_data_processing/results"
...
# download baselayer from natural earth (just for visualization)
...
# grid for the effort estimation 
...
# visualization spatial layers
ggplot() +
  geom_sf(data = coastal_ban_zone, fill = "green") +
  geom_sf(data = worldmap, fill = "gray55") +
  geom_sf(data = port_buf, colour = "red") +
  coord_sf(xlim = c(10, 20), ylim = c(35, 40)) +
  theme_bw() +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black")) +
  ggtitle("Spatial layers")
...
all_dat<-read.csv("AIS_data_processing/data/datatest.csv") # load all data
head(all_dat)

xrange = range(all_dat$longitude)
yrange = range(all_dat$latitude)
ggplot() +
  geom_sf(data = coastal_ban_zone, fill = "green") +
  geom_sf(data = worldmap, fill = "gray55") +
  geom_sf(data = port_buf, colour = "red") +
  coord_sf(xlim = xrange, ylim = yrange) +
  geom_point(data = all_dat, aes(longitude, latitude , colour = as.factor(MMSI)), size = 0.1) +
  theme_bw() +
...
xrange = range(dat_with_trip$longitude)
yrange = range(dat_with_trip$latitude)
ggplot() +
  geom_sf(data = coastal_ban_zone, fill = "green") +
  geom_sf(data = worldmap, fill = "gray55") +
  geom_sf(data = port_buf, colour = "red") +
  coord_sf(xlim = xrange, ylim = yrange) +
  geom_point(data = dat_with_trip, aes(longitude, latitude , colour = as.factor(trip)), size = 0.1) +
  theme_bw() +
  theme(panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(),
        panel.background = element_blank(), 
...
dat_classified=classification_wrapper(vessel_data=dat_with_trip, 
                                      ...
                                      write.output=F,
                                      output.name = "test1")
head(dat_classified[[1]])
head(dat_classified[[2]])
...
ggplot() +
  ...
  geom_sf(data = worldmap, fill = "gray55") +
  geom_sf(data = port_buf, colour = "red") +
  geom_sf(data = fishing_points, colour = ifelse(fishing_points$fishing == 1, "red", "black"), 
             size = ifelse(fishing_points$fishing == 1, 0.5, 0.1)) +
  coord_sf(xlim = xrange, ylim = yrange) +
  theme_bw() +
  theme(panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(),
...
ggplot() +
  ...
  geom_sf(data = worldmap, fill = "gray55") +
  geom_sf(data = port_buf, colour = "red") +
  geom_sf(data = fishing_tracks[[1]], aes(colour = as.factor(trip))) +
  coord_sf(xlim = xrange, ylim = yrange) +
  theme_bw() +
  theme(panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(),
        ...
        axis.line = element_line(colour = "black"),
        legend.position = "bottom") +
  guides(colour = guide_legend(title = "Trip", override.aes = list(size = 1)))
...
ggplot() +
  ...
  geom_sf(data = worldmap, fill = "gray55") +
  geom_sf(data = port_buf, colour = "red") +
  geom_sf(data = vessel_grid[[1]], aes(fill = f_hours)) +
  geom_sf(data = fishing_tracks[[1]]) +
  coord_sf(xlim = xrange, ylim = yrange) +
